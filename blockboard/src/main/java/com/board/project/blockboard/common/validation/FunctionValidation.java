...
public class FunctionValidation {
  ...
  public boolean isFunctionOn(int companyID, int functionID, HttpServletResponse response) {
    boolean isValid = functionService.idUseFunction(companyID, functionID);
    try {
      if (!isValid) {
  ...
  public boolean isFunctionOn(int companyID, int postFunctionID, int commentFunctionID,
    ...
    boolean isPostFunctionValid = functionService.idUseFunction(companyID, postFunctionID);
    boolean isCommentFunctionValid = functionService.idUseFunction(companyID, commentFunctionID);
